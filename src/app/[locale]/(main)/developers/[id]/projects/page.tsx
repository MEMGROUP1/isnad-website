...
export default async function DeveloperProjectsPage({ params }: PageProps) {
    const { id } = await params;
    const t = await getTranslations("projects");
    const tDev = await getTranslations("developers");

    // Fetch cities, complexes, and the developer details
    const [cities, complexes, developer] = await Promise.all([
        websiteService.getCities({ developerId: id }),
        websiteService.getComplexes({ developerId: id }),
        websiteService.getDeveloperById(id).catch(() => null),
    ]);
    ...
    const developerName = developer.name?.ar || developer.name?.en || "";
    ...
    return (
        <Section className="lg:h-auto py-14">
            <Section.Inner>
                <div className="mb-10">
                    <h1 className="text-[32px] md:text-[38px] lg:text-[48px] text-white">{developerName}</h1>
                    {/* Optional: Add developer description or back button if needed */}
                </div>

                {cityCards.length === 0 && complexCards.length === 0 && (
                    <div className="text-white text-xl text-center py-20">{tDev("no_projects_found")}</div>
                )}
